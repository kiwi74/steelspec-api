...
polished, professional steel schedule + connection report.

IMPORTANT DISTINCTION — this is a DATA REPORT, not a technical
drawing: tables, schedules, a cover page, prose. It has no geometry,
no dimensioned orthographic views, no to-scale member elevations.
The future drawing_generator module (Slice C, not built yet) is a
completely different tool that will consume 3D geometry from
cad_engine and produce actual fabrication drawings. Do not conflate
the two, and do not move this module under drawing_generator/ — the
two serve genuinely different purposes and audiences.
"""
...
CONNECTION_TYPE_LABELS = {
    "bolted": "Bolted", "welded": "Welded",
    "bolted_and_welded": "Bolted & Welded", "unspecified": "Unspecified",
}


def _hex(c) -> str:
    return "#{:02x}{:02x}{:02x}".format(int(round(c.red * 255)), int(round(c.green * 255)), int(round(c.blue * 255)))


def _styles():
    styles = getSampleStyleSheet()
    styles.add(ParagraphStyle(name="SectionLabel", fontName="Helvetica-Bold", fontSize=9.5, textColor=RUST, spaceAfter=2 * mm))
    styles.add(ParagraphStyle(name="SectionHeading", fontName="Helvetica-Bold", fontSize=17, textColor=INK, spaceAfter=2 * mm, leading=20))
    styles.add(ParagraphStyle(name="SectionDesc", fontName="Helvetica", fontSize=9.5, textColor=GREY, spaceAfter=5 * mm, leading=14))
    styles.add(ParagraphStyle(name="CellText", fontName="Helvetica", fontSize=8, textColor=INK, leading=11))
    styles.add(ParagraphStyle(name="CellTextMono", fontName="Helvetica", fontSize=7.5, textColor=INK2, leading=10))
    styles.add(ParagraphStyle(name="CellHeader", fontName="Helvetica-Bold", fontSize=7.5, textColor=white, leading=10))
    styles.add(ParagraphStyle(name="CardTitle", fontName="Helvetica-Bold", fontSize=11, textColor=INK, spaceAfter=1 * mm))
    styles.add(ParagraphStyle(name="CardMeta", fontName="Helvetica", fontSize=8, textColor=GREY, leading=12))
    return styles
...
def _styled_table(data, col_widths, has_totals_row=False):
    ...
    if has_totals_row:
        cmds += [("LINEABOVE", (0, -1), (-1, -1), 1.2, INK), ("BACKGROUND", (0, -1), (-1, -1), RUST_BG)]
    table.setStyle(TableStyle(cmds))
    return table


def _metric_card(value: str, label: str, styles, accent=RUST):
    value_p = Paragraph(f'<font color="{_hex(INK)}"><b>{value}</b></font>',
                         ParagraphStyle("MetricValue", fontName="Helvetica-Bold", fontSize=22, leading=26, alignment=TA_CENTER))
    label_p = Paragraph(f'<font color="{_hex(GREY)}">{label.upper()}</font>',
                         ParagraphStyle("MetricLabel", fontName="Helvetica", fontSize=7.5, leading=10, alignment=TA_CENTER))
...
def generate_report_pdf(project_id: str) -> bytes:
    ...
    members = supabase.table("steel_members").select("*").eq("project_id", project_id).order("mark").execute().data
    connections = (
    ...
    buffer = BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=A4, leftMargin=MARGIN, rightMargin=MARGIN, topMargin=22 * mm, bottomMargin=22 * mm)
    story = []
    ...
    story.append(Paragraph(
        "Key figures extracted from the uploaded structural model, calculated automatically against the NZ/AU steel section database.",
        styles["SectionDesc"],
    ))
    ...
    story.append(Paragraph(
        "Every structural steel member extracted from the design file, matched against the steel section database with calculated weights.",
        styles["SectionDesc"],
    ))
    ...
    for m in members:
        ...
        if has_source_pages:
            source = "-"
            if m.get("source_drawing") or m.get("source_page"):
                bits = [b for b in [m.get("source_drawing"), f"p{m['source_page']}" if m.get("source_page") else None] if b]
                source = " ".join(bits) if bits else "-"
            data.append([
                Paragraph(m.get("mark") or "-", styles["CellText"]),
                Paragraph(m.get("section_name") or m.get("section_name_raw") or "?", styles["CellTextMono"]),
                Paragraph(f'{m.get("length_mm"):.0f}' if m.get("length_mm") else "NOT SPECIFIED", styles["CellTextMono"]),
    ...
    story.append(Paragraph(
        f'<font color="{_hex(RUST)}"><b>Total structural steel weight: {total_weight_kg:.0f} kg ({total_weight_kg / 1000:.2f} tonnes)</b></font>',
    ...
    if connections:
        story.append(Paragraph(
            "Connections identified in the design file with bolt, weld, and plate details as specified, cross-referenced to the members they join.",
            styles["SectionDesc"],
        ))
        ...
        story.append(Paragraph(
            "No connection details were identified in this file. Connection extraction (bolts, plates, welds) covers "
            "models where the engineer has specified this information explicitly in the drawing — it may not be "
            "present in every source file.",
            ParagraphStyle("Note", fontName="Helvetica", fontSize=9.5, textColor=GREY, leading=14, backColor=BG,
    ...
    if unmatched:
        ...
        story.append(Paragraph("The following section callouts could not be matched to the steel section database and require manual review:", styles["SectionDesc"]))
        for s in unmatched:
    ...
    if warnings:
        ...
        for w in warnings:
            story.append(Paragraph(f"&bull; {w}", ParagraphStyle(
                "WarningItem", fontName="Helvetica", fontSize=9, textColor=AMBER, leading=13, spaceAfter=2 * mm,
            )))
...
def _connection_card(c: dict, idx: int, styles):
    ...
    members_linked = [cm["steel_members"]["mark"] for cm in (c.get("connection_members") or []) if cm.get("steel_members")]
    members_str = ", ".join(members_linked) if members_linked else "Not linked to a specific member"

    header_row = Table([[Paragraph(f"<b>Connection {idx}</b>", styles["CardTitle"]), _type_pill(conn_type, styles)]],
    ...
    if spec_rows:
        spec_table = Table(
            [[Paragraph(f"<b>{label}</b>", styles["CellText"]), Paragraph(val, styles["CellTextMono"])] for label, val in spec_rows],
